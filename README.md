...
## latex中文去AI
````markdown
去ai味，输出latex格式，不要完全按论文的标准来写，而是要在保证看起来向人写的前提下是文章看起来向论文，不能处处表述地无懈可击，不能夸大其词，也不能过于口语化，偶尔轻微的口语化是可以的。你的目的是让专家和导师看论文时觉得是学生自己写的而不是ai生成的。此外，文章中首次提到的名词缩写使用这样的形式：使用引用标注紧跟术语 示例：卷积神经网络（Convolutional Neural Network, CNN）[11]，名词不是缩写就不用加括号解释了。
不要用括号、分号、破折号的形式，不要说空话、无意义的话（典型的AI表现例如： “人工智能技术在计算机视觉领域的突破，为遥感图像智能解译提供了新的技术方向。”“新的技术方向”这种很虚的表述。还有“具有重要价值” “但回顾研究过程，仍然存在一些问题和有待进一步突破的地方”这种表述也是。以及“将是未来值得深入探索的重要方向”这样的表述。）然后你能不能对我所给内容全部去ai并输出？
````
